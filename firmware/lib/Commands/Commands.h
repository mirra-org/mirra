...
struct CommonCommands
{
    ...
    {
        return std::make_tuple(CommandAliasesPair(&CommonCommands::listFiles, "ls", "list"),
                               CommandAliasesPair(&CommonCommands::printFile, "print", "printfile"),
                               CommandAliasesPair(&CommonCommands::printFileHex, "printhex", "printfilehex"),
                               CommandAliasesPair(&CommonCommands::removeFile, "rm", "remove"), CommandAliasesPair(&CommonCommands::touchFile, "touch"),
                               CommandAliasesPair(&CommonCommands::format, "format"), CommandAliasesPair(&CommonCommands::echo, "echo"),
                               CommandAliasesPair(&CommonCommands::echoNum, "echonum"), CommandAliasesPair(&CommonCommands::exit, "exit", "close"));
    };
};
...
class CommandParser
{
    ...
    template <class Tup, size_t I = 0> CommandCode parseArgs(Tup& argsTuple, const std::array<char*, std::tuple_size_v<Tup>>& buffers);
...
public:
    ...
    template <class N> static typename std::enable_if_t<std::is_arithmetic_v<N>, std::optional<N>> parseNum(const char* buffer);
    template <class N> static typename std::enable_if_t<std::is_arithmetic_v<N>, std::optional<N>> readNum();
    /// @brief Helper function to facilitate the editing of a value (numeric or string) for the user.
