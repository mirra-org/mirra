...
template <MessageType T, class... Args> void Window::emplace(Args&&... args)
{
    ...
    *writeHeadMessage = Message<T>(std::forward<Args>(args)...);
    push(writeHeadMessage->getSize());
}
...
template <MessageType T> const Window::MessagesArray<T>& Protocol::receiveMessages()
{
    receive(sizeof(Message<T>), T);
    return recvWind.getMessagesArray<T>();
}
